...
var CollectionService = (function () {
    function CollectionService(http) {
        this.http = http;
        this.collectionUrl = 'http://localhost:8080/poetryAPI/collections/'; // URL to web API
    }
    CollectionService.prototype.getCollections = function (id) {
        return this.http.get(this.collectionUrl + id)
            .map(this.extractData)
            .catch(this.handleError);
    };
    CollectionService.prototype.createCollection = function (name, id) {
        var collection = new collection_1.Collection();
        collection.name = name;
        collection.writer_id = id;
        var bodyString = JSON.stringify(collection); // Stringify payload
